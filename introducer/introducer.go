...
func (s *server) Introduce(ctx context.Context, in *intro.IntroduceRequest) (*intro.IntroduceReply, error) {
	...
	//Send a Join message to itself for the SDFS server to process
	intrAddress := "172.22.158.181"
	intrAddrwithPort := fmt.Sprintf("%s:%d", intrAddress, globob.SDFS_PORT)
	conn, err := net.Dial("udp", intrAddrwithPort)

	CheckErr(err)

	defer conn.Close()

	joinMsg := generateJoinBuffer(requestorIP)

	_, err = conn.Write(joinMsg)
